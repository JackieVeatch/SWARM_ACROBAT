...
% survey
% 6June2023 copied from 'defining_bloom_final_reprocessed_adelie_transect.m' 
% to recreate integrating to 5m instead of the MLD (Matt's suggestion on paper)
...
load '/Users/jveatch/Documents/MATLAB/SWARM/ACROBAT/data_reprocessed/ACRO_reprocessed5m.mat'
...
for i = 1:length(survey_days)
    ...
    chl = ACRO.five_particle(ind); %%% edit here to change patch-determining variable
...
%% I think this is a better way to define edges: adapted from "compare real to random"
%% STILL A WORKING PROGRESS
% index edges as the first profile of patch +/- 1 and the last profile of patch +/- 1
ind_edges_all = [];
edges = NaN(length(bloom_binary_all),1);
c = 1;
...
patchID_Adelie5m.variable = 'integrated five meter particle backscatter';
patchID_Adelie5m.creationDate = '6-June-2022';
patchID_Adelie5m.createdWith = 'defining_bloom_final_reprocessed_adelie_transect_5m.m';
patchID_Adelie5m.patch = bloom_binary_all;
patchID_Adelie5m.timestamp = bloom_binary_all_timestamp;
patchID_Adelie5m.dens_diff = dens_diff_all;
patchID_Adelie5m.lon = bloom_binary_all_lon;
patchID_Adelie5m.lat = bloom_binary_all_lat;
patchID_Adelie5m.ftle = ftle_all;
patchID_Adelie5m.rpd = rpd_all;
patchID_Adelie5m.edges = edges_all;
patchID_Adelie5m.edges_threshold = edges_threshold_all;